...
prior_flag = 1;
cf = 1;
load( strcat(DiceName,'_prior',num2str(prior_flag),'_cf',num2str(cf),'_', ROI));    
...
interp = 1;
cf = 1;
load( strcat(DiceName,'_cf',num2str(cf),'_interp',num2str(interp), '_', ROI));

% 3. SIMPL
DiceName = 'SIMPL';
Metric = 0;
load( strcat(DiceName,'_Metric',num2str(Metric),'_', ROI))

% 4. MajorVoting
DiceName = 'Dice_MajorVoting';
load( strcat(DiceName, '_', ROI))
...
DiceName = 'AMR';
SN = 50;
load( strcat('Dice_', DiceName, '_LW_SkullStripped_', ROI, '_SN_', num2str(SN) ) );
